...
<br> 
<?php
$statement = $pdo->prepare("SELECT COUNT(*) AS anzahl FROM mechakten");
$statement->execute();  
$row = $statement->fetch();
...
<br>
<?php
$sql = "SELECT * FROM mechakten";
foreach ($pdo->query($sql) as $row) {
   echo "Auftragsnummer: ".$row['ID']." "."<strong>Vorname: </strong>".$row['vor']." "."<strong>Nachname: </strong>".$row['nach']."<strong> Auftragsdatum: </strong>".$row['time_of_act']."<br />";
...
error_reporting(0);
$search = $_POST["search"];
$sql = "DELETE FROM mechakten WHERE ID = $search" ;
foreach ($pdo->query($sql) as $row) {
